...
Existing general purpose search engines are restrictive centralized databases everybody is forced to trust. Those search engines was designed primary for client-server architecture based on DNS, HTTP and IP protocols. Emergence of content-addressable storage and distributed ledger technology create opportunity for the Internet to acquire new features such as more efficient computing, storing and broadband consumption, more resilience, security and access privacy, and elimination of middlemen for digital property. This can shift existing client-server architecture ubiquitously used on the web to a truly peer-to-peer interactions based on stateless IPFS and variety of stateful consensus computers such as Ethereum. This creates a challenge and an  opportunity for a search engine based on emerging technologies and specifically designed for them. Surprisingly the blockchain architecture itself allows to organize general purpose search engine in a way that was inaccessible for previous architectures. In this paper we discuss opportunities behind blockchain based search engine, challenges of crawling, indexing, and evaluation for the next generation web and propose a blockchain based experimental set of smart contracts to address discussed issues.

## Introduction

Let us start the discussion from listing the disadvantages of conventional general purpose search engines:

- No Transparency. Nobody outside of Google understands how the ranking really works. That creates a market for black and white SEO. The truth is that if e.g. Google discloses complete details of ranking algorithm it would be easy for malicious parties (adversaries as it were) to game organic search results that would kill the quality of results and ad revenue streams. Pagerank [PR] has no inherent trust mechanism resistant to sybil attacks. This problem can be addressed by adding a transparent and accountable blockchain based ledger with properly designed economic incentives built into the system.
- No Access. Currently all search engines are centralized. Nobody is able to add to the index as well as participate in improving quality of search results. However, Google itself internally uses a workforce of _search evaluators_. It is our belief that user generated search engine could have higher quality of results as the case with almost every web site with user generated content in existence.
- Broken Incentives. Vast majority of contribution to a search quality is made by users. When any user searches for something they extend the semantic core. When any user clicks on search result they train the model {train the algorithm? smth else}. This creates an opportunity to continuously improve ranking model at the expense of the users. Then search engines sell users to advertisers at the expense of user experience and acquire revenue streams which are not returned back to the users at all. This simple loop created Alphabet's $550 billion capitalization (~$80 per capita of Earth) in 18 years. We want to change that.
- Central Control. Google became too powerful. It is scary to imagine a future there _everything_ about _everybody_ is known and controlled by a **closed** AI corporation. Imagine the world there (1) the only one country exists, (2) nobody can control it's government and (3) everybody has to obey the decisions of government without any explanation. There should be an open, transparent and accessible _alternative_ with _decentralized control_ built on principles of modern distributed interplanetary content-addressable cyberspace [IPFS] and DAO like governance [RALF].
- Annoying Ads. Separation for organic and ad search results is unnecessary. In fact all organic ranking decisions are being made by search authority. But for paid search results Google uses a free market solution to determine the fair ad price for every word in its gigantic semantic core. Historically the free market solutions are more efficient in virtually any area of decision making. Why not use the same principles for ranking itself, thus disintermediating the advertising industry? Let us imagine that every link can be (1) curated or audited by everybody, (2) based on this trusted cyber•rank metric (page rank based on economically incentivized curation and auditing) is calculated and then (3) everybody can promote this link further by burning some money, which automatically brings value for everybody in existence. For every action everybody earns a share proportionally to their contributions. This non-zero-sum game is significantly more sybil resistant and that is there we are heading.
- One Way Trust. Everybody has to trust Google, Baidu and Yandex. But Google, Baidu and Yandex don't trust users. E.g. you cannot report that a link is fradulent even with some kind of proof and so should not be ranked as highly. It {I would clarify this it. What is it? Search authority?} can measure your attention during ranking but it can also reject that metric. You cannot know what happens inside because Google, Baidu and Yandex don't trust us. We want to establish a system where trust is bidirectional between search engine and users. It is achieved by distributing search engine ownership across all it's users based on contribution to the data upon which all ranking decisions are made.
- Zero Privacy. All search engines will answer you only if they explicitly know how to map your device with your real identity or pseudo identity which is tracked by RTB [RTB]. Otherwise you have to prove that you are not a robot every time you search. That harms our privacy. Moreover, robot abuse is another hot topic that is about to become relevant. Nonetheless, privacy should be paramount.
- Censorship. Though it's well known that Google is working hard to prevent censorship we all know about China [GCHINA](https://en.wikipedia.org/wiki/Google_China) case and [Transparency Report](https://www.google.com/transparencyreport/). Good search provider should be resistant to censorship without exceptions and built for interplanetary scale in mind.
- Online only. It is worthwhile to note that you cannot search offline even if necessary information is stored next door. If we are cut from the wire or backbone we are powerless. Global offline search is not a feature which can be easily deployed even by multibillion corporation. This goal is nearly impossible to achieve based on centralized architecture. Only accessible distributed systems can solve this fundamental problem for the next generation Internet. This future is not about gateway keepers in form of ISPs but about mesh networking and peer-to-peer communications.
- Weak Security. What happens if tomorrow my Google account will be blocked? Do we have something to prevent this? Do I have necessary level of assurance that _guarantees_ us our security based on math and not on complicated legal tender {??? legal tender = money}? All technical solutions are here but to solve this important issue we need to do a lot of work because security is a foundation for life, liberty and property. {I would rephrase that.}
